...
from .const import CONF_POLL_INTERVAL, CONF_TRACKED_ENTITY_ID, DEFAULT_POLL_INTERVAL_MINUTES, DOMAIN
...
class BuienwatchDataUpdateCoordinator(DataUpdateCoordinator[BuienwatchData]):
    ...
    async def _async_update_data(self) -> BuienwatchData:
        """Fetch the current location, poll both sources, and combine them."""
        ...
        buienradar_result, buienalarm_result = await asyncio.gather(
            async_fetch_buienradar(self._session, lat, lon),
            async_fetch_buienalarm(self._session, lat, lon),
            return_exceptions=True,
        )
        buienradar_samples = self._unwrap(buienradar_result, "Buienradar")
        buienalarm_samples = self._unwrap(buienalarm_result, "Buienalarm")

        if buienradar_samples is None and buienalarm_samples is None:
            raise UpdateFailed("Both Buienradar and Buienalarm are unavailable")
